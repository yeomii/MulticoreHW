...
__kernel void main(__constant double* ppdFactors,
                   ...
                   __global double* gpdSwapPayoffs,
                   ...
                   const long lTrials)
{
  ...
  for (int i = 0; i < iSwapVectorLength; i++)
   pdSwapPayoffs[i] = gpdSwapPayoffs[gid*iSwapVectorLength + i];
...
/*
  pdSwaptionPrice[id*2*gs + gid*2] = dSumSimSwaptionPrice/lTrials; // MeanPrice
  pdSwaptionPrice[id*2*gs + gid*2+1] = sqrt(fabs((dSumSquareSimSwaptionPrice-dSumSimSwaptionPrice*dSumSimSwaptionPrice/lTrials)/(lTrials-1.0)))/
