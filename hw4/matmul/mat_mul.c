	...
	for(ii = 0; ii < NDIM; ii += B){
		for (jj = 0; jj < NDIM; jj += B){
			for (kk = 0; kk < NDIM; kk +=B){
				...
				for( i = ii; i < MIN(NDIM, (ii+B)); i++ ){
					for( j = jj; j < MIN(NDIM, (jj+B)); j++ ){
						for( k = kk; k < MIN(NDIM, (kk+B)); k++ ){
...
void parse_opt(int argc, char** argv)
{
	...
	while( (opt = getopt(argc, argv, "pvhikjs:b:")) != -1 )
	{
		switch(opt)
		{
		case 'p':
